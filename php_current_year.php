...
function incomes ($connect, $id , $value, $month, $year) {
	...
	$result = $connect->query("SELECT SUM(amount) FROM incomes AS inc INNER JOIN incomes_category_assigned_to_user AS ic ON inc.income_category_assigned_to_user_id = ic.id AND ic.name = '$value' AND inc.user_id = ic.user_id AND YEAR(date_of_income) = '$year'");
...
function expenses ($connect, $id , $value,$month, $year) {
	...
	$result = $connect->query("SELECT SUM(amount) FROM expenses AS exp INNER JOIN expenses_category_assigned_to_user AS ec ON exp.expenses_category_assigned_to_user_id = ec.id AND ec.name = '$value' AND exp.user_id = ec.user_id AND YEAR(date_of_expense) = '$year'");
